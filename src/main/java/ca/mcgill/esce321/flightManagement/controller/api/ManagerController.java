package ca.mcgill.esce321.flightManagement.controller.api; // <- keep consistent with your folders
...
public class ManagerController {
    ...
    // CREATE
    @PostMapping
    public ResponseEntity<ManagerResponse> create(@RequestBody ManagerRequestDTO request) {
        final ManagerResponse created = managerService.createManager(request);
    ...
    // READ one
    @GetMapping("/{id}")
    public ResponseEntity<ManagerResponse> getById(@PathVariable("id") long id) {
    ...
    // UPDATE
    @PutMapping("/{id}")
    public ResponseEntity<ManagerResponse> update(@PathVariable("id") long id,
    ...
    public ResponseEntity<FlightResponse> assignFlight(
            @PathVariable long flightId,
            @RequestBody List<Long> employeeIds
    ) {
        FlightResponse success = managerService.assignFlight(flightId, employeeIds);
        return ResponseEntity.ok(success);
    }

    @GetMapping("/flights")
    public ResponseEntity<List<FlightResponse>> viewAllFlights() {
        List<FlightResponse> flights = managerService.viewAllFlights();
        return ResponseEntity.ok(flights);
    }

    @GetMapping("/flights/{flightId}/stats")
    public ResponseEntity<FlightResponse> viewFlightStats(@PathVariable long flightId) {
        FlightResponse stats = managerService.viewFlightStats(flightId);
    ...
    @GetMapping("/bookings")
    public ResponseEntity<List<BookingResponse>> viewAllBookings() {
        List<BookingResponse> bookings = managerService.viewAllBookings();
    ...
    @GetMapping("/persons")
    public ResponseEntity<List<PersonResponse>> viewAllPersons() {
        List<PersonResponse> persons = managerService.viewAllPersons();
